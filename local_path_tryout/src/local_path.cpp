...
local_path::local_path()
{
    ...
    pnh.param<double>("path_param",avoid_path_param, 1.0);
    pnh.param<double>("avoid_mindist",avoid_mindist, 0.8);
    pnh.param<double>("avoid_objdist",avoid_objdist, 5.0);
...
void local_path::static_object(sensor_msgs::PointCloud object, nav_msgs::Path path, nav_msgs::Path lp, nav_msgs::Path rp)
{
    if(object.points.size() > 0)
    {
        //check left
        std::pair<int, double> index_dist = calc_dist(object, lp);
        ...
        //check right
        index_dist = calc_dist(object, rp);

        min_dist = index_dist.second;
        closest_index = index_dist.first;

        x = object.points.at(closest_index).x;
        y = object.points.at(closest_index).y;

        obj_distance = sqrt(x*x + y*y);
...
void local_path::path_tracking(nav_msgs::Path global_path)
{
    if(flag == true)
    {
        ...
        //left tracking path
        Ltracking_path.header.stamp = ros::Time::now();
        Ltracking_path.header.frame_id = "map";
        Ltracking_path.poses.clear();
        //right tracking path
        Rtracking_path.header.stamp = ros::Time::now();
        Rtracking_path.header.frame_id = "map";
        Rtracking_path.poses.clear();
        ...
        if(temp_num + num <= global_path.poses.size())
        {
            for(int i = temp_num; i< temp_num + num; i++)
            {
                ...
                Ltracking_path.poses.push_back(temp_pose);

                temp_pose.pose.position.x = global_path.poses.at(i).pose.position.x + avoid_path_param*sin(vehicle_yaw);
                temp_pose.pose.position.y = global_path.poses.at(i).pose.position.y - avoid_path_param*cos(vehicle_yaw);
                temp_pose.pose.position.z = 0;
                Rtracking_path.poses.push_back(temp_pose);
        ...
        {
            tracking_path = Rtracking_path;
            path_pub.publish(tracking_path);
        }
        else if(left == false && right == true)
        {
            tracking_path = Ltracking_path;
            path_pub.publish(tracking_path);
        }
...
void local_path::process()
{
    ...
    if(flag == true)
    {
        ...
        if(is_look_foward_point == true)
        {
            ...
            if(state == "static")
            {
                static_object(object_point, tracking_path, Ltracking_path, Rtracking_path);
            }
